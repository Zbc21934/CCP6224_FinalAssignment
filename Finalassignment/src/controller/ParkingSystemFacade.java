...
public class ParkingSystemFacade {
    ...
    public ParkingSystemFacade() {
        ...
        this.reportService = new ReportService(this.conn, this.parkingLot, this.fineManager);
    ...
    public Ticket parkVehicle(String plateNumber, String vehicleType, String spotId, boolean isHandicapped, boolean hasReservation) {
        ...
        if (isViolation) {
            ...
            if (fineAmount > 0) {
                recordFineToDb(plateNumber, fineAmount, "Reserved Spot Misuse");
            }
        }
    ...
    public String getDashboardReport(String reportType) {
        switch (reportType) {
            case "OCCUPANCY":
                return reportService.getOccupancySummary();
            case "REVENUE":
                return reportService.getRevenueSummary();
            case "VEHICLES":
                return reportService.getActiveVehicleTable();
            case "FINES":
    ...
    // store fine to db
    private void recordFineToDb(String plateNumber, double amount, String reason) {
        String sql = "INSERT INTO fines (plate_number, amount, reason, status) VALUES (?, ?, ?, 'UNPAID')";
        try (PreparedStatement pstmt = conn.prepareStatement(sql)) {
            pstmt.setString(1, plateNumber);
            pstmt.setDouble(2, amount);
            pstmt.setString(3, reason);
            pstmt.executeUpdate();
            System.out.println("Fine saved to DB for " + plateNumber);
        } catch (SQLException e) {
